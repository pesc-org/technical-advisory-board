...
<!-- start schema-->
	<xsl:template match="/xs:schema">
		...
		<xsl:for-each select="./xs:element|./xs:simpleType|./xs:complexType">
	...
	<xsl:template name="process-element">
		...
		<!--wc3 type-->
		<xsl:choose>
			<xsl:when test="exists($element/xs:complexType)">
					<xsl:text>&quot;type&quot;: &quot;object&quot;,</xsl:text>
					<xsl:text>&quot;properties&quot;: {</xsl:text>
					<xsl:call-template name="process-complex-type">
...
<!--processes simple-types to json-->
	<xsl:template name="process-simple-type">
		...
		<xsl:choose>
			...
			<xsl:when test="exists($simple-type//xs:list)">
				<xsl:text>&quot;type&quot;:&quot;array&quot;</xsl:text>
				<xsl:if test="$simple-type//xs:list/@itemType">
					<xsl:text>,&quot;items&quot;:{&quot;type&quot;:&quot;</xsl:text>
					<xsl:variable name="list-type" select="substring-after($simple-type//xs:list/@itemType,'.')"/>
					<xsl:choose>
						<xsl:when test="$list-type='string'">
							<xsl:text>string</xsl:text>
						</xsl:when>
						<xsl:when test="$list-type='boolean'">
							<xsl:text>boolean</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xs:text>number</xs:text>
						</xsl:otherwise>
					</xsl:choose>
					<xsl:text>&quot;}</xsl:text>
				</xsl:if>
...
<!-- template process-documentation-->
	<xsl:template name="process-documentation">
		<xsl:param name="elem"/>
		<xs:text>&quot;description&quot;:&#32;&quot;</xs:text>
		<xsl:if test="exists($elem//xs:documentation)">
			<xsl:value-of select="translate($elem/xs:annotation/xs:documentation[1],'&quot;' , '')"/>
		</xsl:if>
		<xsl:text>&quot;,&#32;</xsl:text>
	</xsl:template>
...
<!-- template: process-complex-types-->
	<xsl:template name="process-complex-type">
		...
		<xsl:if test="exists($complex-type//xs:attribute)">
			<xsl:for-each select="$complex-type//xs:attribute">
			...
			<xsl:if test="$complex-type//xs:sequence|$complex-type/xs:choice">
				<xsl:text>,</xsl:text>
			</xsl:if>
		</xsl:if>
		<xsl:if test="exists($complex-type/xs:simpleContent/xs:extension/@base)">
			<xsl:text>,&quot;value&quot;:{</xsl:text>
			<xsl:variable name="base" select="$complex-type//xs:extension/@base"/>
			<xsl:choose>
				<xsl:when test="substring-before($base,':')='xs'">
					<xsl:text>&quot;type&quot;:</xsl:text>
					<xsl:call-template name="process-w3c-type">
						<xsl:with-param name="attribute-value" select="$complex-type//xs:extension/@base"/>
					</xsl:call-template>
					<xsl:call-template name="process-facets">
						<xsl:with-param name="simple-type" select="$complex-type//xs:extension/@base"/>
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>&quot;$ref&quot;:&quot;#/definitions/</xsl:text>
					<xs:value-of select="$simple-type//xs:extension/@base"/>
					<xsl:text>&quot;</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:text>}</xsl:text>
...
<!-- template recursively adds extension elements to list of elements-->
	<xsl:template name="build-extensions">
		...
		<xsl:variable name="base">
			<xsl:choose>
				<xsl:when test="contains($complex-type/xs:complexContent/xs:extension/@base,':')">
					<xsl:value-of select="substring-after($complex-type/xs:complexContent/xs:extension/@base,':')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$complex-type/xs:complexContent/xs:extension/@base"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="next-type" select="/xs:schema/xs:complexType[@name=$base]"/>
		<xsl:choose>
			<xsl:when test="exists($next-type/xs:complexContent/xs:extension/@base)">
				<xsl:call-template name="build-extensions">
...
<!-- processes each element in the element list for a complex type-->
	<xsl:template name="process-element-list">
		...
		<xsl:text>,&quot;additionalProperites&quot;: false</xsl:text>
	</xsl:template>

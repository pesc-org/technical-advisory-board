...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	<!--takes a simple type and outputs a  xs: type or a xs:restriction base mapping as well as the the facets (e.g., minLength,maxLength,minium, maximum, mininumExclusive, etc), and enumerations-->
	<xsl:template match="/xs:schema/xs:simpleType">
		<xsl:if test="not(./@name =/xs:schema/element()[1]/@name)">
			<xsl:text>,</xsl:text>
		</xsl:if>
		<xsl:if test="exists(./@name)">
			...
			<xsl:text>&quot;:{</xsl:text>
			<xsl:choose>
				<xsl:when test="exists(.//xs:restriction/@base)">
					<xsl:variable name="base" select="data(.//xs:restriction/@base)"/>
					<xsl:choose>
						<xsl:when test="substring-before($base,':')='xs'">
							<xsl:text>&quot;type&quot;:</xsl:text>
							<xsl:call-template name="process-w3c-type">
								<xsl:with-param name="attribute-value" select=".//xs:restriction/@base"/>
							</xsl:call-template>
						...
						<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="exists(.//xs:restriction/element())">
									<xsl:call-template name="process-facets">
										<xsl:with-param name="simple-type" select="."/>
									</xsl:call-template>
								</xsl:when>
	...
	<!-- outputs facets to JSON for restriction base-->
	...
	<xsl:template name="get-simple-type">
		<xsl:param name="simple-type-name"/>
	</xsl:template>
	<!-- go through all complex types-->
	<xsl:template match="/xs:schema/xs:complexType">
		...
		<xsl:variable name="element-list">
			<xsl:sequence select=".//xs:attribute"/>
		...
		<xsl:call-template name="output-element-list">
	...
	<!-- Assumes that all elements have a type defintion. Local definitions and references not supported-->
	<xsl:template name="output-element-list">
	...
	<xsl:template name="process-element">
		...
		<xsl:choose>
			<xsl:when test="substring-before($element/@type,':')='xs'">
				<xsl:text>&quot;type&quot;:</xsl:text>
				<xsl:call-template name="process-w3c-type">
					<xsl:with-param name="attribute-value" select="data(./@type)"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>

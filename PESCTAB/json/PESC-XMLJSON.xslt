...
<xsl:stylesheet version="2.0"
  ...
  <!-- added by PESC to include the prefix on tag names -->
  <xsl:param name="append-prefix" as="xs:boolean" select="true()"/>
  ...
  <xsl:function name="json:create-string" as="node()">
    ...
    <xsl:choose>
      ...
      <!-- added by PESC to have prefix on name -->
      <xsl:when test="$append-prefix">
		  <json:name><xsl:value-of select="$node/name()"/></json:name>
      </xsl:when>
      <xsl:otherwise>
